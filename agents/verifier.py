...
`supported | partial | unsupported`, and attaches currency flags via an
Anthropic tool-use loop terminating on `submit_verification`.

Invariants:
- Fail-closed: loop-cap exhaustion, missing submission, or malformed input
  returns an all-`unsupported` VerificationResult. The orchestrator's
  verifier-retry-then-refuse path turns that into CITATIONS_UNVERIFIABLE.
- Invariant coercion: `feedback_for_drafter` must be non-null iff any claim
  is `unsupported`. If Claude omits it we synthesize feedback from the claim
  reasoning rather than fail hard.
- Verifier judges; it never rewrites the draft.
"""
...
def _parse_submit_input(
    ...
    if result.has_unsupported and not result.feedback_for_drafter:
        ...
        # Frozen model; round-trip through model_validate.
        ...
        # Drop stray feedback: orchestrator keys off `has_unsupported`.
        patched = result.model_dump()
...
class OpusVerifier:
    ...
    async def run(self, state: QueryState) -> VerificationResult:
        ...
        # cache hits within the 5min TTL. Min cacheable prefix on Opus 4.7 is
        # 4096 tokens; tools + system clear that.
        system_blocks = [
            {
                "type": "text",
        ...
        # Opus 4.7 / Sonnet 4.6: adaptive thinking only.
        ...
        while iterations < MAX_LOOP_ITERATIONS:
            ...
            # Heartbeat: a single turn under 1M-ctx + xhigh can run 30-60s.
            state.append_trace(
                trace(
                    "verifier",
                    "thinking",
